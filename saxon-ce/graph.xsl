...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" extension-element-prefixes="ixsl" xmlns="http://www.w3.org/1999/xhtml" xmlns:a="http://ajax.org/2005/aml" version="2.0" exclude-result-prefixes="#all" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:math="http://www.w3.org/2005/xpath-functions/math" xmlns:op="http://robbertatwork.com/2012/operator" xmlns:js="http://saxonica.com/ns/globalJS" xmlns:ixsl="http://saxonica.com/ns/interactiveXSLT" xmlns:style="http://saxonica.com/ns/html-style-property" xmlns:x="http://xslt2.org/extensions" xmlns:sig="http://xslt2.org/signature" xmlns:ev="http://www.w3.org/2001/xml-events" sig:value="1e91e6aa8e386" xmlns:m="http://www.w3.org/1998/Math/MathML">
  ...
  <xsl:template match="a:axis">
    ...
    <ol class="axis z-axis">
      <xsl:for-each select="xs:integer($x1) to xs:integer($x2)">
        <li>
          <xsl:value-of select="."/>
        </li>
      </xsl:for-each>
  ...
  <xsl:template match="a:graph">
    ...
    <xsl:variable name="stepsX" select="0 to (xs:integer(($maxX - $minX) div $precisionX) - 1)"/>
    <xsl:variable name="stepsZ" select="0 to (xs:integer(($maxZ - $minZ) div $precisionZ) - 1)"/>
    ...
    <xsl:for-each select="$stepsX">
      ...
      <xsl:for-each select="$stepsZ">
        ...
        <xsl:variable name="rotateX" select="if ($measure3 = $measure4) then 0 else if ($measure3 &lt; $measure4) then 10 else -10"/>
        <xsl:variable name="rotateY" select="if ($measure1 = $measure2) then 0 else if ($measure1 &gt; $measure2) then 10 else -10"/>
  ...
  <xsl:template name="ripple">
    ...
    <xsl:for-each select="x:class(ixsl:page(), 'square')">
      ...
      <xsl:variable name="measure3" select="js:formula($x, $z - 0.1, $time)"/>
      <xsl:variable name="measure4" select="js:formula($x, $z + 0.1, $time)"/>
      <xsl:variable name="rotateX" select="if ($measure3 &lt; $measure4) then 10 else if ($measure3 = $measure4) then 0 else -10"/>
      <xsl:variable name="rotateY" select="if ($measure1 &gt; $measure2) then 10 else if ($measure1 = $measure2) then 0 else -10"/>
      <!-- The following instructions cause errors because -vendorprefix-transform isn't a valid QName. I guess the pseudo attribute approach isn't sufficient. -->
      <!--
      <ixsl:set-attribute name="style:-moz-transform" select="concat('rotateX(', $rotateX, 'deg) rotateY(', $rotateY, 'deg) translateZ(', $y * 65, 'px)')"/>
      <ixsl:set-attribute name="style:-webkit-transform" select="concat('rotateX(', $rotateX, 'deg) rotateY(', $rotateY, 'deg) translateZ(', $y * 65, 'px)')"/>
      <ixsl:set-attribute name="style:-ms-transform" select="concat('rotateX(', @data-rotateX, 'deg) rotateY(', @data-rotateY, 'deg) translateZ(', $y * 65, 'px)')"/>
      -->
    </xsl:for-each>
  </xsl:template>

  <!-- getElementsByClassName() equivalent -->
  <xsl:function name="x:class" use-when="not(function-available('x:class'))">
    <xsl:param name="contextItem"/>
    <xsl:param name="className"/>
    
    <!-- This implementation isn't complete, but it works for my uses -->
    <xsl:sequence select="js:getElementsByClassName($contextItem, $className)"/>
  </xsl:function>
  ...
  <xsl:function name="x:id" use-when="not(function-available('x:id'))">
    ...
    <!-- This implementation isn't complete, but it works for my uses -->
    <xsl:sequence select="js:getElementById($idref)"/>
  </xsl:function>
